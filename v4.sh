...
show_header() {
  clear_screen
  echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}║${NC}              TERMUX ZSH INSTALLER - FULL CUSTOMIZATION           ${CYAN}║${NC}"
  echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════╝${NC}\n"
}
...
install_dependencies() {
  ...
  gem install lolcat 2>/dev/null || echo -e "${YELLOW}⚠️  lolcat installation skipped (optional)${NC}"
...
install_ohmyzsh() {
  ...
  if [[ -d "$OHMYZSH_DIR" ]]; then
    echo -e "${YELLOW}⚠️  Oh-My-Zsh sudah terinstall. Backup ke ~/.oh-my-zsh.bak${NC}"
    mv "$OHMYZSH_DIR" "${OHMYZSH_DIR}.bak"
  fi
  
  # Install Oh-My-Zsh tanpa mengubah shell otomatis
  echo -e "${BLUE}▶ Downloading Oh-My-Zsh...${NC}"
  sh -c "$(curl -fsSL https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" "" --unattended
  
  echo -e "\n${GREEN}✅ Oh-My-Zsh installed!${NC}"
  sleep 2
}
...
input_name() {
  ...
  echo -e "\n${GREEN}✅ Nama disimpan: ${YELLOW}$USER_NAME${NC}"
  sleep 2
}
...
show_banner_menu() {
  ...
  echo -e "${YELLOW}[1]${NC} ${GREEN}☠️  Skull Banner${NC}      ${YELLOW}[5]${NC} ${GREEN}🦋 Butterfly Banner${NC}"
  echo -e "${YELLOW}[2]${NC} ${GREEN}🕷️  Spider Banner${NC}     ${YELLOW}[6]${NC} ${GREEN}🎲 Random Banner${NC}"
  echo -e "${YELLOW}[3]${NC} ${GREEN}🐉 Dragon Banner${NC}"
  echo -e "${YELLOW}[4]${NC} ${GREEN}🏴‍☠️  One Piece Banner${NC}"
  
  if [[ "$show_extra_options" == "yes" ]]; then
    echo -e "${YELLOW}[7]${NC} ${PURPLE}👁️  Preview Banner${NC}"
    echo -e "${YELLOW}[8]${NC} ${PURPLE}🔄 Ganti Nama${NC}"
    echo -e "${YELLOW}[9]${NC} ${RED}❌ Keluar${NC}"
  fi
...
preview_banner() {
  ...
  clear_screen
  echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}║${NC}                     ${WHITE}PREVIEW BANNER${NC}                         ${CYAN}║${NC}"
  echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════╝${NC}\n"
  
  case $choice in
    1) 
      echo -e "${GREEN}☠️  SKULL BANNER${NC}\n"
      banner_skull_demo
      banner_name="Skull"
      ;;
    2) 
      echo -e "${GREEN}🕷️  SPIDER BANNER${NC}\n"
      banner_spider_demo
      banner_name="Spider"
      ;;
    3) 
      echo -e "${GREEN}🐉 DRAGON BANNER${NC}\n"
      banner_dragon_demo
      banner_name="Dragon"
      ;;
    4) 
      echo -e "${GREEN}🏴‍☠️  ONE PIECE BANNER${NC}\n"
      banner_onepiece_demo
      banner_name="One Piece"
      ;;
    5) 
      echo -e "${GREEN}🦋 BUTTERFLY BANNER${NC}\n"
      banner_butterfly_demo
      banner_name="Butterfly"
      ;;
    6)
      echo -e "${GREEN}🎲 RANDOM BANNER${NC}\n"
...
select_banner() {
  ...
  while $running; do
    ...
    if [[ "$choice" =~ ^[1-6]$ ]]; then
      ...
      echo -e "\n${GREEN}✅ Banner terpilih: ${YELLOW}$BANNER_NAME${NC}"
      running=false
      sleep 2
    else
  ...
  while $running; do
    ...
    case $choice in
      1|2|3|4|5|6)
        ...
        echo -e "\n${GREEN}✅ Banner diganti: ${YELLOW}$BANNER_NAME${NC}"
      ...
      8)
        ...
        if [[ -n "$new_name" ]]; then
          ...
          echo -e "\n${GREEN}✅ Nama diganti menjadi: ${YELLOW}$new_name${NC}"
          echo -e "${YELLOW}Perubahan akan aktif setelah restart Termux${NC}"
        else
...
show_date() {
  ...
  echo -e "\n\033[1;91m        ╼═\033[94m☾ \033[93mDATE\033[95m: \033[4;97m$hari\033[0m\033[94m , \033[4;92m$tgl\033[0m\033[95m-\033[4;92m$bln\033[0m\033[95m-\033[4;92m$thn\033[0m\033[94m ☽\033[91m═╾"
}
...
EOF
  ...
  echo -e "${GREEN}✅ File banner created: ${YELLOW}$BANNER_FILE${NC}"
  sleep 2
}
...
create_zshrc() {
  ...
  if [[ -f "$ZSHRC" ]]; then
    ...
    echo -e "${YELLOW}⚠️  Backup .zshrc ke .zshrc.backup${NC}"
  fi
...
PROMPT='%{\033[0m%}%{\033[2;95m%}╭━═━❨%{\e[1;94m%}${USER}%{\033[93m%}㉿%{\033[1;96m%}%m%{\033[2;95m%}❩═━═❨%{\033[42;91m%} %~ %{\033[0m%}%{\033[2;95m%}❩══╾%{\033[1;94m%}[ %* ]%{\033[2;95m%}
║ 
%{\033[2;95m%}╰━═%{\e[1;31m%}❯%{\e[1;33m%}❯%{\e[1;36m%}❯%{\033[0m%} %{\033[2;92m%}'
...
EOF
  ...
  echo -e "${GREEN}✅ .zshrc berhasil dikonfigurasi${NC}"
  sleep 2
}
...
change_shell() {
  ...
  if ! grep -q "$(command -v zsh)" /etc/shells 2>/dev/null; then
    ...
    echo -e "${YELLOW}⚠️  Tidak bisa menulis ke /etc/shells${NC}"
  fi
  
  # Ganti shell
  chsh -s zsh
  
  echo -e "\n${GREEN}✅ Default shell diganti ke ZSH!${NC}"
...
show_summary() {
  show_header
  echo -e "${GREEN}╔══════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║${NC}                     INSTALASI SELESAI!                           ${GREEN}║${NC}"
  echo -e "${GREEN}╚══════════════════════════════════════════════════════════════════╝${NC}\n"
  ...
  # Nama user
  USER_NAME=$(<"$NAME_FILE")
  echo -e "  ${WHITE}• Nama: ${CYAN}$USER_NAME${NC}"
  ...
  if [[ -f "$CHOICE_FILE" ]]; then
    ...
    echo -e "  ${WHITE}• Banner: ${CYAN}$BANNER_NAME${NC}"
  fi
  
  # Komponen terinstall
  echo -e "  ${WHITE}• Shell: ${CYAN}ZSH + Oh-My-Zsh${NC}"
  echo -e "  ${WHITE}• Plugins: ${CYAN}autosuggestions, syntax-highlighting, completions${NC}"
  echo -e "  ${WHITE}• Lokasi banner: ${CYAN}$BANNER_FILE${NC}"
  echo -e "  ${WHITE}• Lokasi config: ${CYAN}$ZSHRC${NC}\n"
  
  echo -e "${PURPLE}💡 Menu Konfigurasi Pasca-Instalasi:${NC}"
  echo -e "  ${YELLOW}Jalankan ulang script ini untuk mengakses menu:${NC}"
  echo -e "  ${CYAN}./install-zsh.sh${NC}\n"
  
  echo -e "${GREEN}✅ INSTALASI SELESAI!${NC}"
  echo -e "${YELLOW}⚠️  RESTART TERMUX AGAR PERUBAHAN BERLAKU${NC}\n"
...
check_existing_installation() {
  if [[ -f "$CHOICE_FILE" ]] || [[ -f "$NAME_FILE" ]] || [[ -f "$BANNER_FILE" ]] || [[ -d "$OHMYZSH_DIR" ]]; then
    show_header
    echo -e "${YELLOW}⚠️  Deteksi instalasi yang sudah ada!${NC}\n"
    echo -e "  ${WHITE}• File konfigurasi ditemukan${NC}"
    echo -e "  ${WHITE}• Oh-My-Zsh sudah terinstall${NC}\n"
    ...
    case $existing_choice in
      1)
        ...
        echo -e "${GREEN}✅ Backup selesai! Melanjutkan instalasi...${NC}"
        sleep 2
        return 0
        ;;
      2)
